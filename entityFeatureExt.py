...
def get_feature(self):
    ...
    for i,word in enumerate(self.row['sentence'].split(" ")):
        features = {}
        features["Bias"] = 1.0
        features["word"] = word
        ...
        if i>0:
            prev_word = self.row["sentence"].split(" ")[i-1]
        ...
        if i<len(self.row["sentence"].split(" "))-1:
            next_word = self.row["sentence"].split(" ")[i+1]
